# Step 2: SBERT 임베딩 재계산 (Ubuntu 4090에서 실행)

## 목적
Kushi 데이터의 `paraphrase-MiniLM-L3-v2` 임베딩을 `all-MiniLM-L6-v2`로 재계산.
동시에 mood 라벨을 정규화한 텍스트로 임베딩해서, 감성 구조가 더 잘 나오는지 확인.
...
`step2_reembed.py`를 아래 내용으로 생성하고 실행해줘.
...
```bash
source .venv/bin/activate
python step2_reembed.py
```

4090에서 1,275개 텍스트 × 2 = 약 10초 이내.

## 완료 후
`data/step2/` 폴더에 .npy 파일이 생기면 완료.
다음 단계(Step 3: go_emotions)는 별도 프롬프트 참조.
