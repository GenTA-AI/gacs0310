...
export class FeedbackTrackerService {
  async updateActualScores() {
    const { rows } = await pool.query(
      ...
      RETURNING pl.id, pl.book_id, pl.predicted_priority_score, pl.actual_priority_score`,
    );
  ...
  async computeDriftMetrics() {
    const { rows } = await pool.query(
      `SELECT
         COUNT(*) AS total,
         AVG(ABS(pl.predicted_priority_score - pl.actual_priority_score)) AS mae,
         SQRT(AVG(POWER(pl.predicted_priority_score - pl.actual_priority_score, 2))) AS rmse,
         CORR(pl.predicted_priority_score, pl.actual_priority_score) AS pearson_r
